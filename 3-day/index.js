...
app.put("/kitap-guncelle/:kitap_id", async (req, res, next) => {
  const kitap_id = req.params.kitap_id;
  const body = req.body;
  // const kitap = await Kitap.findById(kitap_id);

  // kitap.adi='Değişiklik';
  // kitap.isbn='1321321321';
  // await kitap.save();

  const kitap = await Kitap.findOneAndUpdate(
    { _id: kitap_id },
    { adi: body.adi, isbn: body.isbn },
    { new: true }
  );
  res.send(kitap);
});

app.delete("/kitap-sil/:kitap_id", async (req, res, next) => {
  const kitap_id = req.params.kitap_id;
  const kitap = await Kitap.findByIdAndDelete(kitap_id);
  return res.send(kitap);
});
